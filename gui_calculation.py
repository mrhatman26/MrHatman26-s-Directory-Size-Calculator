...
def gui_calculation(selected_directory, byte_type):
        ...
        if os.path.isdir(sd):
            ...
            for dirpath, dirnames, filenames in os.walk(sd):
                for f in filenames:
                    ...
                    gui_calculating_file_log.yview("end")
                    gui_calculating_file_log.config(yscrollcommand=gui_calculating_file_log_scroll.set)
                    gui_calculating_file_log_scroll.update()
                    gui_calculating_file_log.configure(state="disabled")
    ...
    gui_calculating_file_log_scroll = t.Scrollbar(window, command=gui_calculating_file_log.yview, width=15)
    gui_calculating_file_log_scroll.place(x=737, y=53, height=167)
